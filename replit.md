...
## Changelog
- June 23, 2025: **ADMIN DASHBOARD READABILITY COMPLETELY RESOLVED** - Fixed all remaining readability issues across admin panel including tab navigation and subscription tier dropdowns, resolved critical tab readability where inactive tabs displayed unreadable light gray text on dark backgrounds by implementing proper white text contrast with subtle gray backgrounds, fixed edit haunt form subscription tier dropdown with proper bg-gray-800 border-gray-600 dark styling and white text hover states, added missing activeTab state variable with proper onValueChange handler, ensured consistent professional appearance across all 7 admin dashboard tabs (Management, Haunts, Packs, Assignments, Default Ads, Branding, Analytics) while maintaining red-to-purple gradient horror theme throughout
