...
## Changelog
- June 24, 2025: **GLOBAL STYLES MIGRATED TO TAILWIND CONFIG** - Completely migrated all global fonts (Creepster, Nosifer, Eater) and horror theme colors (blood, void, shadow, flame, ghost, spirit, poison, crimson, dark, correct, wrong) from index.css to tailwind.config.ts with proper theme.extend structure, added all custom animations (pulse-glow, shake, fade-in, lightning-flash, glitch-lines, sprite-glitch-in) with keyframes, cleaned index.css to only contain Tailwind directives and Google Font imports, eliminated all redundant custom utility classes and global body styles for better maintainability and Tailwind v4 compatibility
