...
## Changelog
- June 24, 2025: **PREVIEW HORROR STYLING COMPLETELY RESTORED** - Fixed preview version to correctly load client/tailwind.config.ts horror theme by adding CSS variables (:root with --blood, --shadow, --void, etc.), implementing explicit horror utility classes (.bg-gradient-dark, .glass-card, .font-creepster), adding Google Fonts import for Creepster/Nosifer/Eater typography, including comprehensive keyframe animations, and removing problematic PostCSS configuration conflicts - preview now displays identical horror styling to deployed version with complete dark blue gradients, red-to-purple answer buttons, spooky fonts, and atmospheric animations
