...
## Changelog
- June 23, 2025: **ADMIN DASHBOARD READABILITY ISSUES COMPLETELY RESOLVED** - Identified and fixed CSS layer specificity conflict causing subscription tier dropdown text to be unreadable despite multiple styling attempts, root cause was @layer system in index.css where base layer styles were overriding component-level Select styling, moved all Radix UI Select styling rules into @layer components section ensuring proper cascade hierarchy, all subscription tier dropdowns now display white text on dark gray backgrounds throughout admin dashboard, eliminated weeks-long readability issue that persisted across multiple sessions
