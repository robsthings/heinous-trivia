...
## Changelog
- June 23, 2025: **ADMIN DASHBOARD TAB READABILITY COMPLETELY FIXED** - Resolved critical tab readability issue where inactive tabs displayed unreadable light gray text on dark backgrounds, implemented proper contrast with white text and subtle gray backgrounds for inactive tabs while maintaining red-to-purple gradient horror theme for active tabs, added missing activeTab state variable with proper onValueChange handler, ensured consistent professional appearance across all 7 admin dashboard tabs (Management, Haunts, Packs, Assignments, Default Ads, Branding, Analytics) with full horror theme integration
