...
## Changelog
- June 20, 2025: **EQUAL DISTRIBUTION SYSTEM IMPLEMENTED** - Fixed question loading to pull equally and randomly from all assigned packs per haunt instead of sequential loading, implemented sourcePackId tracking for proper pack attribution, created questionsPerPack calculation (Math.ceil(20 / packIds.length)) ensuring equal representation from each assigned trivia pack, enhanced both haunt-specific and fallback pack assignment logic with equal distribution, questions now randomly selected from each pack then combined for balanced gameplay
