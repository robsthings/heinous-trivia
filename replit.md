...
## Changelog
- June 23, 2025: **ADMIN DASHBOARD READABILITY ISSUES FULLY RESOLVED** - Fixed all subscription tier dropdown readability problems with comprehensive CSS overrides using important declarations to force dark backgrounds (rgb(31, 41, 55)) with white text on all SelectContent elements, resolved critical tab readability where inactive tabs displayed unreadable light gray text by implementing proper white text contrast with subtle gray backgrounds, fixed both main haunt management and edit haunt form subscription tier dropdowns with proper styling, added missing activeTab state variable with proper onValueChange handler, ensured consistent professional appearance across all 7 admin dashboard tabs while maintaining red-to-purple gradient horror theme throughout
